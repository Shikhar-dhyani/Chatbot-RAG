...
app.config['SAVED_FILES']='archived_files'
...
@login_required
def logout():
    logger.info(f'{current_user.username} wants to logout')
    logout_user()
...
def login():
    ...
    if current_user.is_authenticated:
        logger.info(f'User {current_user.username} attempted to access login page while already authenticated.')
...
def create_user():
    if not current_user.role == 'admin':
        logger.warning(f'Unauthorized access attempt to create_user route by user: {current_user.username}')
    ...
    if request.method == 'POST':
        ...
        if not all([username, password, role]):
            logger.error('Incomplete data received while creating user.')
            flash('Missing data', 'error')
            return redirect(url_for('create_user'))
        
        if db.session.query(User.id).filter_by(username=username).first():
            logger.warning(f'User creation failed: User with username "{username}" already exists.')
        ...
        try:
            ...
            flash('An error occurred while creating user. Please try again later.', 'error')
            return redirect(url_for('create_user'))
...
def upload_file_or_uri():
    ...
    if not current_user.role == 'admin':
        logger.warning(f'Unauthorized access attempt to upload route by user: {current_user.username}')
        return "Unauthorized", 403  # Redirect or return a different response for unauthorized access
    ...
    if not files and not uri:
        logger.error('No file or URI provided in the request.')
    ...
    logger.info(f'Files uploaded: {uploaded_files} , Files skipped: {skipped_files} by {current_user.username}')
    logger.info(f'Errors encountered: {errors} in uploading files')
...
def load():
    if current_user.role != 'admin':
        logger.warning(f'Unauthorized access attempt to load route by user: {current_user.username}')
    ...
    for file_name in allowed_files:
        ...
        try :
            ...
            logger.info(f'{current_user.username}: Embeddings loaded into Vector DB for file: {file_name}')
            shutil.move(file_path, os.path.join(archive_directory, file_name))
            logger.info(f'{current_user.username}: File moved to archive directory: {file_name}')

        except Exception as e:
            print(" ")
            logger.error(f'Error loading file: {file_name}. Error: {e}')
...
def response_query():
    ...
    if not document_name or not query_text:
        logger.info(f"{current_user.username} made an incomplete query.")
    ...
    if not ids_to_lookup:
        logger.info("No matches found for this document")
    ...
    try:
        ...
        if connection.is_connected():
            ...
            context = " ".join(id_to_text.get(id, " ") for id in ids_to_lookup)
            answer = get_answer(context)  
            if not answer:
                logger.info("No answer found for this query")
            else :
